...
/* 
	...
	Attributes:
		...
		$link   = string option specifying the href value for optional link: 'parent', '/custom/url/', or '' (default is empty = no link, image only)
...
/*
	...
	Parameters:
		...
		$link   = string option specifying the href value for link (leave empty/false for image only) -> default = false (other options: 'file', 'attachment', 'parent', 'image', '/custom/url/', or '')
...
/*
	...
	Parameters: exactly the same as for get_posts() : see @ https://codex.wordpress.org/Template_Tags/get_posts
		Plus:
...
/* 
	Function: display user avatar
	Displays avatar for current logged-in user
	Shortcode: [usp_user_avatar size="80"]
*/
if (!function_exists('usp_user_avatar')) : 
function usp_user_avatar($attr) {
	extract(shortcode_atts(array('size' => 80), $attr));
	$current_user = wp_get_current_user();
	if (!$current_user->exists()) return;
	return get_avatar($current_user->ID, $size);
}
add_shortcode('usp_user_avatar', 'usp_user_avatar');
endif;
...
/*
	...
	Docs @ https://plugin-planet.com/usp-pro-display-list-submitted-posts/
	
	More @ https://codex.wordpress.org/Class_Reference/WP_Query#Category_Parameters
...
function usp_pro_display_posts($attr, $content = null) {
	...
	if ((!is_user_logged_in()) && ($loggedin === 'true' || $userid === 'logged')) return;
	...
	$noposts = apply_filters('usp_display_posts_noposts', '<p>'. esc_html__('No posts found.', 'usp-pro') .'</p>');
	if (empty($submitted_posts)) return $noposts;
