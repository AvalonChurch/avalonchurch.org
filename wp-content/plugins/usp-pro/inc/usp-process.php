...
if (!class_exists('USP_Pro_Process')) {
	...
	class USP_Pro_Process {
		...
		public function insert_post() {
			...
			if (usp_is_session_started() === false) usp_session_start();
		...
		public function post_tags($fields, $default_tags) {
			...
			if (!empty($fields['usp_tags'])) {
				...
				if (is_array($user_tags)) {
					...
					$use_id = apply_filters('usp_post_tags_use_id', true);
					foreach ($user_tags as $user_tag) {
						if (is_numeric($user_tag) && $use_id) {
							$post_tag = get_term_by('id', intval($user_tag), 'post_tag', ARRAY_A);
							$post_tags .= $post_tag['name'] . ', ';
						} else {
							$post_tags .= $user_tag . ', ';
		...
		public function insert_attachments($fields, $user_id, $post_id) {	
			global $usp_uploads;
			...
			if ($files !== '') {
				...
				if (isset($file_data['name']) && is_countable($file_data['name'])) {
					...
					for ($i = 0; $i < count($file_data['name']); $i++) {
						...
						if (!empty($file_data['name'][$i])) $file_name = apply_filters('usp_get_file_name', sanitize_file_name(basename($file_data['name'][$i])));
						else continue;
						...
						if (stripos($file, 'php') === false) $bytes = file_put_contents($file, $file_local);
						// @chmod($file, 0644);
						...
						if ($featured_image == 1) {
							...
							if (isset($file_data['key'][$i])) $current_key = intval($file_data['key'][$i]);
							else $current_key = null;
							
							if (($i + 1) == $image_key || $current_key == $image_key) {
		...
		public static function regex_filter($string, $args) {
			...
			$post_slug = empty($post_slug) ? $post_id : $post_slug;
		...
		public function get_usp_files_data($usp_files, $files_key, $files_value) {
			...
			if (isset($files_value['name'])) {
				$count = is_countable($files_value['name']) ? count($files_value['name']) : 1;
				for ($i = 0; $i < $count; $i++) {
		...
		public function process_files() {
			...
			if (isset($_FILES) && !empty($_FILES)) {
				...
				if (isset($tmp_name) && is_countable($tmp_name)) {
		...
		public function set_session_vars() {
			...
			if (isset($usp_general['sessions_on'])) {
				...
				if (isset($usp_general['sessions_default']) && !empty($usp_general['sessions_default'])) setcookie('remember', 'remember', time() + 60*60*24*3); // 3 days
				if (isset($_POST['usp-remember'])) {
					setcookie('forget', '', 1);
					setcookie('remember', 'remember', time() + 60*60*24*3);
				} else {
					setcookie('remember', '', 1);
					setcookie('forget', 'forget', time() + 60*60*24*3);
				}
			}
		}
		...
		public function get_field_val() {
			...
			if (isset($_POST['usp_form_submit']) && empty($_POST['usp-verify']) && wp_verify_nonce($_POST['usp_form_submit'], 'usp_form_submit')) {
				...
				} elseif (isset($_POST['recaptcha_response']) && !empty($_POST['recaptcha_response'])) {
					$recaptcha_secret = isset($usp_general['recaptcha_private']) ? $usp_general['recaptcha_private'] : null;
					$recaptcha = file_get_contents('https://www.google.com/recaptcha/api/siteverify?secret='. $recaptcha_secret .'&response='. $_POST['recaptcha_response']);
					$recaptcha = json_decode($recaptcha);
					$score = apply_filters('usp_recaptcha_score', 0.5);
					$pass = (($recaptcha->success == true) && ($recaptcha->score >= $score)) ? true : false;
					if (!$pass) $error_5 = 'usp_error_5a';
				} else {
